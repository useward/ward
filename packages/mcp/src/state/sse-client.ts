      ...
      this.eventSource.onerror = (_error: Event) => {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
          ...
          this.options.onError(
            new Error(`Failed to connect to NextDoctor DevTools at ${this.options.url} after ${this.maxReconnectAttempts} attempts`)
          )
        }
      }

      this.eventSource.addEventListener("client-traces", (event: MessageEvent) => {
        this.handleTraceEvent(event.data, "client")
      })

      this.eventSource.addEventListener("server-traces", (event: MessageEvent) => {
        this.handleTraceEvent(event.data, "server")
      })

      this.eventSource.addEventListener("navigation-event", (event: MessageEvent) => {
        this.handleNavigationEvent(event.data)
      })
    } catch (error) {
      this.options.onError(
        new Error(`Failed to connect to NextDoctor DevTools at ${this.options.url}: ${error}`)
      )
    }
  }
