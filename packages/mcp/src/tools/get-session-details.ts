...
const formatResource = (
  ...
  const name = resource.name.length > 60 ? resource.name.substring(0, 57) + "..." : resource.name
  lines.push(
...
const formatResourceTree = (
  ...
  for (const resource of resources) {
    ...
    if (resource.children.length > 0) {
      lines.push(...formatResourceTree(resource.children, baseTime, depth + 1, includeAttrs))
    }
  }
  return lines
}

export const getSessionDetails = (store: SessionStore, args: GetSessionDetailsArgs): string => {
...
Use get_sessions tool to see all available sessions.`
  ...
  lines.push(`Type: ${session.navigationType === "initial" ? "Initial Load" : "SPA Navigation"}`)
  ...
  if (timing.serverStart !== undefined && timing.serverEnd !== undefined) {
    lines.push(`  Server: ${formatDuration(timing.serverEnd - timing.serverStart)}`)
  }
  ...
  if (session.rootResources.length > 0) {
    ...
    const serverResources = session.rootResources.filter((r) => r.origin === "server")
    const clientResources = session.rootResources.filter((r) => r.origin === "client")
