import { Search, Server, Monitor, AlertTriangle, X } from "lucide-react"
  ...
  return (
    <div className="px-4 py-2 border-b border-border space-y-2 bg-card/30">
      <div className="flex items-center gap-3">
        ...
        <button
          type="button"
          onClick={() => onFiltersChange({ showErrorsOnly: !filters.showErrorsOnly })}
          className={cn(
            "flex items-center gap-1 px-2 py-1 text-[10px] rounded border transition-colors",
            filters.showErrorsOnly
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input
            ...
            onChange={(e) => onFiltersChange({ minDuration: Number(e.target.value) || 0 })}
          />
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive ? cn(cfg.bg, cfg.border, cfg.text) : "bg-background border-border text-muted-foreground hover:bg-accent"
              )}
            >
              {cfg.label}
            </button>
