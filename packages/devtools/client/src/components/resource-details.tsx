    ...
    <div className="fixed right-0 top-0 h-full w-[400px] bg-card border-l border-border shadow-xl z-50 flex flex-col">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className={cn("text-xs font-medium px-2 py-0.5 rounded", config.bg, config.text)}>{typeConfig.label}</span>
          <h3 className="font-semibold text-sm truncate max-w-[200px]" title={resource.name}>
            {resource.name}
          </h3>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 space-y-4">
          ...
          <TimingSection resource={resource} sessionStart={session.timing.navigationStart} />

          {resource.url && <UrlSection url={resource.url} />}

          <AttributesSection attributes={resource.attributes} />

          {resource.children.length > 0 && <ChildrenSection children={resource.children} />}
        </div>
      </div>
    </div>
  )
...
function findResourceById(resources: ReadonlyArray<Resource>, id: string | null): Resource | null {
  if (!id) return null
...
function SummarySection({ resource }: { resource: Resource }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      />
      <InfoCard
        icon={resource.origin === "server" ? <Server className="size-3.5 text-green-400" /> : <Monitor className="size-3.5 text-blue-400" />}
        label="Origin"
        value={originConfig.label}
      />
      <InfoCard
        icon={resource.status === "error" ? <AlertTriangle className="size-3.5 text-red-400" /> : <CheckCircle className="size-3.5 text-green-400" />}
        label="Status"
        value={resource.status === "error" ? "Error" : "OK"}
        valueClassName={resource.status === "error" ? "text-red-400" : "text-green-400"}
      />
      {resource.statusCode !== undefined && (
        <InfoCard
          ...
          valueClassName={resource.statusCode >= 400 ? "text-red-400" : resource.statusCode >= 300 ? "text-amber-400" : "text-green-400"}
        />
      )}
      {resource.size !== undefined && (
        <InfoCard icon={<Database className="size-3.5" />} label="Size" value={formatBytes(resource.size)} />
      )}
      {resource.cached && (
        <InfoCard icon={<Database className="size-3.5 text-teal-400" />} label="Cache" value="Cached" valueClassName="text-teal-400" />
      )}
    </div>
  )
}
...
function TimingSection({ resource, sessionStart }: { resource: Resource; sessionStart: number }) {
    ...
    <div className="space-y-2">
      ...
      <div className="bg-accent/20 rounded-lg p-3 space-y-2">
        <div className="flex justify-between text-xs">
          ...
          <span className="font-mono">{formatDuration(relativeStart)} from nav start</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">End</span>
          <span className="font-mono">{formatDuration(relativeEnd)} from nav start</span>
        </div>
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">Duration</span>
          <span className="font-mono font-medium">{formatDuration(resource.duration)}</span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="bg-accent/20 rounded-lg p-3">
        <p className="text-xs font-mono break-all text-muted-foreground">{url}</p>
      </div>
    </div>
  )
}

function AttributesSection({ attributes }: { attributes: Record<string, string | number | boolean> }) {
  const entries = Object.entries(attributes).filter(([key]) => !key.startsWith("nextdoctor."))
          ...
          <tbody>
            {entries.map(([key, value]) => (
              <tr key={key} className="border-b border-border/30 last:border-0">
                <td className="px-3 py-1.5 text-muted-foreground font-mono truncate max-w-[150px]" title={key}>
                  {key}
                </td>
      ...
      <div className="space-y-1">
        {children.slice(0, 10).map((child) => {
          ...
          return (
            ...
            >
              <div className={cn("w-1 h-4 rounded-sm", child.origin === "server" ? "bg-green-500" : "bg-blue-500")} />
              <span className={cn("text-[10px] font-medium", config.text)}>{RESOURCE_TYPE_CONFIG[child.type].label}</span>
              <span className="flex-1 text-xs truncate">{child.name}</span>
              <span className="text-[10px] font-mono text-muted-foreground">{formatDuration(child.duration)}</span>
            </button>
          )
        })}
        {children.length > 10 && (
          <div className="text-[10px] text-muted-foreground text-center py-1">+{children.length - 10} more</div>
        )}
      </div>
    </div>
  )
