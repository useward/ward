...
import type { PageSession, NavigationType, Resource, ResourceFilterState } from "@/domain"
  ...
  const filteredSessions = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (s) => s.route.toLowerCase().includes(query) || s.url.toLowerCase().includes(query)
      )
    }
      ...
      <div className="px-3 py-2 border-b border-border space-y-2">
        ...
        <div className="flex items-center gap-1 text-xs">
          <select
            ...
            value={filterType}
            onChange={(e) => onFilterChange(e.target.value as NavigationType | "ALL")}
          >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <Badge className={cn("font-mono text-[9px] px-1.5", config.bgColor)}>{config.label}</Badge>
        <span className={cn("flex-1 text-xs truncate", isInitial ? "font-semibold" : "text-muted-foreground")}>
          {session.route}
        </span>
        <span className="text-xs font-mono text-muted-foreground">{formatDuration(session.stats.totalDuration)}</span>
      </div>
      <div className="flex items-center gap-2 mt-1 text-[10px] text-muted-foreground">
        ...
        {hasError && <span className="text-red-400 font-medium">{session.stats.errorCount} err</span>}
      </div>
    </button>
  )
}
...
function SessionDetailPanel({
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">Select a session to view its resources</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      <SessionHeader session={session} showDetails={showDetails} onToggleDetails={onToggleDetails} />
        ...
        <div className="px-4 py-2 bg-amber-500/10 border-b border-amber-500/30">
          <div className="flex items-center gap-2 text-xs text-amber-300">
            ...
            <span className="font-medium">
              {dataFetchCount} data {dataFetchCount === 1 ? "fetch" : "fetches"} for this page
            </span>
          </div>
        </div>
      )}
      ...
      <ScrollArea className="flex-1 p-4">
        <SessionWaterfall session={session} filteredResources={filteredResources} zoom={zoom} />
        <div className="mt-4">
          <ResourceTree session={session} filteredResources={filteredResources} />
        </div>
      </ScrollArea>
    </div>
  )
...
function SessionHeader({ session, showDetails, onToggleDetails }: SessionHeaderProps) {
  const config = NAVIGATION_TYPE_CONFIG[session.navigationType]
    ...
    <div className="px-4 py-3 border-b border-border bg-card">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          <Badge className={cn("font-mono", config.bgColor)}>{config.label}</Badge>
      ...
      <div className="flex items-center gap-6 text-sm">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono font-semibold">{formatDuration(session.stats.totalDuration)}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Layers className="size-4 text-muted-foreground" />
          <span className="font-mono">{session.stats.totalResources} resources</span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        {session.stats.slowestResource && (
          <div className="flex items-center gap-1.5 text-amber-400 ml-auto">
            ...
            <span className="text-xs">
              Slowest: <span className="font-mono">{session.stats.slowestResource.name.substring(0, 30)}</span> (
    ...
    <div className="mt-3 pt-3 border-t border-border/50 grid grid-cols-2 gap-4">
      <div>
        ...
        <div className="space-y-1 text-xs">
          {session.timing.serverStart !== undefined && session.timing.serverEnd !== undefined && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">Server Processing</span>
              <span className="font-mono">{formatDuration(session.timing.serverEnd - session.timing.serverStart)}</span>
            </div>
          )}
          {session.timing.responseStart !== undefined && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">TTFB</span>
              <span className="font-mono">{formatDuration(session.timing.responseStart)}</span>
            </div>
          )}
          {session.timing.domContentLoaded !== undefined && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">DOMContentLoaded</span>
              <span className="font-mono">{formatDuration(session.timing.domContentLoaded)}</span>
            </div>
          )}
          {session.timing.load !== undefined && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">Load</span>
              <span className="font-mono">{formatDuration(session.timing.load)}</span>
            </div>
          )}
        </div>
      </div>
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">URL</h3>
