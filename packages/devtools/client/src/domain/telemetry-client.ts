...
export const TelemetryConnectionError = (
  message: string,
...
const parseTraceData = (
  data: string,
  origin: SpanOrigin,
): Option.Option<TelemetryEvent> => {
  try {
...
const parseNavigationEventFromString = (
  data: string,
): Option.Option<NavigationEvent> => {
  try {
...
export class TelemetryClientService extends Context.Tag(
...
> =>
  Effect.gen(function* () {
    ...
    const offerEvent = (event: TelemetryEvent) =>
      Effect.runSync(Queue.offer(eventQueue, event));
    const offerNavigation = (event: NavigationEvent) =>
      Effect.runSync(Queue.offer(navigationQueue, event));
    const offerConnection = (status: boolean) =>
      Effect.runSync(Queue.offer(connectionQueue, status));
