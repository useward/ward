...
import type { TraceSpan, SpanOrigin, SpanCategory, SpanStatus } from "./types"
import type { OTLPExportTraceServiceRequest, OTLPSpan, KeyValue, AnyValue } from "./otel-types"
...
const parseAttributes = (attrs: ReadonlyArray<KeyValue> | undefined): Record<string, string | number | boolean> => {
  if (!attrs) return {}
  return pipe(
    attrs,
...
const normalizeCategory = (category: string | undefined): SpanCategory | undefined => {
...
const inferCategory = (name: string, attrs: Record<string, string | number | boolean>): SpanCategory => {
  const nameLower = name.toLowerCase()

  if (attrs["http.method"] || attrs["http.request.method"] || nameLower.includes("fetch") || nameLower.includes("http")) {
    return "http"
  }

  if (attrs["db.system"] || nameLower.includes("database") || nameLower.includes("query") || nameLower.includes("prisma")) {
    return "database"
  }
  ...
  if (nameLower.includes("render") || nameLower.includes("rsc") || nameLower.includes("component")) {
    return "render"
  }
...
const nanoToMs = (nano: string): number => Math.floor(parseInt(nano, 10) / 1_000_000)

export const convertOTLPSpan = (span: OTLPSpan, origin: SpanOrigin): RawSpan => {
...
export const groupSpansByTraceId = (spans: ReadonlyArray<RawSpan>): Map<string, RawSpan[]> => {
...
export const groupSpansByRequestId = (spans: ReadonlyArray<RawSpan>): Map<string, RawSpan[]> => {
  const result = new Map<string, RawSpan[]>()
  for (const span of spans) {
    if (span.requestId) {
...
export const buildSpanTree = (spans: ReadonlyArray<TraceSpan>): TraceSpan | undefined => {
  if (spans.length === 0) return undefined

  const spanMap = new Map(spans.map((s) => [s.id, { ...s, children: [] as TraceSpan[] }]))
  const spanIdSet = new Set(spans.map((s) => s.id))

  for (const span of spanMap.values()) {
    if (span.parentId && spanMap.has(span.parentId)) {
      const parent = spanMap.get(span.parentId)!
      parent.children = [...parent.children, span].sort((a, b) => a.startTime - b.startTime)
    }
  }

  const root = [...spanMap.values()].find((s) => !s.parentId || !spanIdSet.has(s.parentId))
  return root ?? [...spanMap.values()].sort((a, b) => a.startTime - b.startTime)[0]
}
